    ...
    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2, alignItems: "center", justifyContent: "space-between" }}>
      <Stack spacing={0.3}>
        ...
        <Stack direction="row" spacing={1} alignItems="center" sx={{ flexWrap: "wrap" }}>
          {typeof roundId === "number" && (
            <Chip label={`Round #${roundId}`} size="small" />
          )}
          {roundStatus && (
            <Chip label={roundStatus} size="small" color={roundStatus === "RUNNING" ? "warning" : "default"} />
          )}
          {start && (
      ...
      <Stack direction="row" spacing={2} alignItems="center">
        <FormControl size="small" sx={{ minWidth: 180 }}>
          ...
          <Select label="Game" value={gameType} onChange={(e) => setGameType(e.target.value as GameType)}>
        ...
        <FormControl size="small" sx={{ minWidth: 150 }}>
          ...
          <Select label="Budget" value={budget} onChange={(e) => setBudget(Number(e.target.value) as any)}>
