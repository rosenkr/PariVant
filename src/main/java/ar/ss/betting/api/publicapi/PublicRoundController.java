...
public class PublicRoundController {
    ...
    public ResponseEntity<CurrentRoundResponse> getCurrent(
            @RequestParam(name = "gameType", required = false) String gameTypeParam
    ) {
        ...
        if (gameTypeParam != null && !gameTypeParam.isBlank()) {
            ...
            if (selected == null) return ResponseEntity.notFound().build();
    ...
    private SelectedRound selectForType(GameType type, LocalDateTime now) {
        List<GameRoundEntity> running =
                gameRoundRepository.findRunningByGameType(type, now, PageRequest.of(0, 1));
        if (!running.isEmpty()) {
            return new SelectedRound(running.get(0), RoundStatus.RUNNING);
        }

        List<GameRoundEntity> upcoming =
                gameRoundRepository.findNextUpcomingByGameType(type, now, PageRequest.of(0, 1));
        if (!upcoming.isEmpty()) {
    ...
    private CurrentRoundResponse toResponse(GameType selectedGameType, SelectedRound selected) {
        ...
        List<MatchEntity> matchEntities =
                matchRepository.findByGameRoundIdOrderByMatchNumberAsc(round.getId());

        List<MatchContextEntity> ctxEntities =
                matchContextRepository.findByGameRoundIdOrderByMatchNumberAsc(round.getId());
